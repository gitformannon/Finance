...
class GoalsCubit extends Cubit<GoalsState> {
  ...
  Future<void> create(String name, int targetAmount, {String? targetDate, int initialAmount = 0, String? emoji_path}) async {
    emit(state.copyWith(loading: true, error: null));
    final res = await _createGoal(CreateGoalRequest(name: name, targetAmount: targetAmount, targetDate: targetDate, initialAmount: initialAmount, emoji_path: emoji_path));
  ...
  Future<void> update({required String id, String? name, int? targetAmount, String? targetDate, String? emoji_path}) async {
    if (_updateGoal == null) return;
    final res = await _updateGoal!(UpdateGoalRequest(id: id, name: name, targetAmount: targetAmount, targetDate: targetDate, emoji_path: emoji_path));
    res.fold((l) => null, (updated) {
