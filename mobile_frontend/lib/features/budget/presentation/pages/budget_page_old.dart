...
class _BudgetPageState extends State<BudgetPage> with TickerProviderStateMixin {
  ...
  late final GetTransactionsByQuery _getTransactionsByQuery;
  ...
  void initState() {
    ...
    _getAccounts = getItInstance<GetAccounts>();
    _getTransactionsByQuery = getItInstance<GetTransactionsByQuery>();
    _getCategories = getItInstance<GetCategories>();

    context.read<BudgetCubit>().load(DateTime.now());
    _accountsListCubit = AccountsListCubit(_getAccounts)..load();
    _loadTotalBalance();
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: LinearGradient(
                ...
                colors: [
                  AppColors.def.withValues(alpha: 0.25),
                    ...
                    builder: (context, state) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) => Padding(
                          key: ValueKey('goal-${state.goals[index].id ?? state.goals[index].name}-${index}'),
                          padding: const EdgeInsets.only(bottom: 12),
        ...
        child: FloatingActionButton(
          ...
          onPressed: () {
            if (_selectedTopTab == 0) {
              ...
              AddAccountModal.show(context).then((_) => _loadTotalBalance());
            } else if (_selectedTopTab == 2) {
              // Budget -> Add purchase category (with budget)
              AddCategoryModal.show(context, type: CategoryType.purchase).then((_) => _loadExpenseCategories());
...
class _BalanceChipState extends State<_BalanceChip> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(color: AppColors.def.withValues(alpha: 0.2), width: 2.0),
    ...
    return ClipRRect(
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(AppSizes.borderSM16),
            // Glass effect: translucent gradient + subtle border + soft shadow
            gradient: LinearGradient(
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
              colors: [
            ...
            children: [
              ...
              Container(
                child: IconButton(
                  ...
                  icon: const Icon(Icons.edit, size: AppSizes.editIcon),
                  onPressed: () async {
                    final changed = await showModalBottomSheet(
                      context: context,
                      useSafeArea: true,
...
class _GoalCardBudgetTab extends StatelessWidget {
  ...
  const _GoalCardBudgetTab({super.key, required this.goal});
